  ...
  async _mockDataByFileMiddware(ctx, next) {
    if (!ctx.path.startsWith('/mock-switch') &&
      ...
      if (this.$cache.hasOwnProperty(path)) {
        ...
        const params = ctx.request.method.toLowerCase() === 'get' ? ctx.request.query : ctx.request.body
        if (switchData) {
          this.$cache[path] = processMock(mockHandle(params), switchData.selections[0].value)
          ctx.body = this.$cache[path]
        } else {
          // 不用mock-switch，正常返回
